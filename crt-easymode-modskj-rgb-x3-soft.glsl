...
#define TARGET_GAMMA_INPUT 1.9
#define TARGET_GAMMA_OUTPUT 2.0
#define TARGET_BRIGHT_BOOST 1.1
...
#		endif
		{
			...
			if (dot_no == 0) {
				mask_weight = vec3(1.05, mask, mask);
			}
			else
			if (dot_no == 1) {
				mask_weight = vec3(mask, 1.03, mask);
	
			}
			else {
				mask_weight = vec3(mask, mask, 1.02);
			}
